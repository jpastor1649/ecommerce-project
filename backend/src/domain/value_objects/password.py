...
class Password:
    ...
    def _validate_strength(password: str) -> None:
        ...
        if len(password) < Password.MIN_LENGTH:
            raise ValueError(f"Password must be at least {Password.MIN_LENGTH} characters long")
        ...
        if not re.search(f"[{re.escape(Password.SPECIAL_CHARS)}]", password):
            raise ValueError(f"Password must contain at least one special character: {Password.SPECIAL_CHARS}")
